...
int http_conn::m_epollfd=-1;//所有socket事件都注册到同一个epoll对象中
...
// 网站的根目录
const char* doc_root = "/home/mywebserver/resources";
...
void addfd(int epollfd,int fd,bool one_shot){
    ...
    //oneshot事件：使得即使socket连接上有多个事件，也不会发生多个线程同时处理socket连接的异常。不过在处理完一个事件后需要重新注册oneshot
    if(one_shot){
...
void http_conn::init(){
    ...
    m_checked_idx=0;//判断一行是否解析到行末
    m_start_line=0;
    m_read_idx=0;
    m_url=nullptr;//请求目标文件的文件名
    m_version=nullptr;//协议版本，只支持1.1
    m_method=GET;//请求方法
    m_linger=false;
    m_content_length=0;
    m_host=nullptr;
...
http_conn::HTTP_CODE http_conn::process_read(){
    ...
    HTTP_CODE ret=NO_REQUEST;//报文解析结果
    char *text;
    while(((m_check_state==CHECK_STATE_CONTENT)&&(line_status==LINE_OK))
    ||(line_status=parse_single_line())==LINE_OK){
        //解析到了一行完整的数据，或者解析到了请求体，也是完整的数据
        //获取一行数据
        text=get_line();
        m_start_line=m_checked_idx;
        std::cout<<"got 1 line http info:"<<text<<std::endl;
...
//解析HTTP请求行。获取请求方法，获取URL，获取HTTP版本
http_conn::HTTP_CODE http_conn::parse_request_line(char * text){
    ...
    *m_version++='\0';
    ...
    m_check_state=CHECK_STATE_HEADER;//主状态机的状态改变
    return NO_REQUEST;
}
...
//解析每一行。从状态机。判断依据:\r\n
http_conn::LINE_STATUS http_conn::parse_single_line(){
    char tmp;
    //检查索引 小于 读索引
